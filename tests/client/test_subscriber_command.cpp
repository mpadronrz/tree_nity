...
TEST(ClientSubscriberCommand, DetectsUnavailableServerDuringRegistration) {
    ...
    command.type = client::CommandType::Subscribe;
    ...
    EXPECT_EQ(client::execute_subscriber_command(command, output, error_output), 1);
    EXPECT_TRUE(output.str().empty());
    EXPECT_NE(error_output.str().find("failed to connect"), std::string::npos);
}
