...
// Añade una cadena con su tamaño para formar un record entregado al consumidor.
void append_string(std::vector<unsigned char>& bytes, const std::string& value) {
    append_uint32(bytes, static_cast<std::uint32_t>(value.size()));
...
TEST(ClientSubscriptionIo, ReadsCompleteSubscriptionMessage) {
    ...
    append_uint32(bytes, 12U);
    append_string(bytes, "user.create");
